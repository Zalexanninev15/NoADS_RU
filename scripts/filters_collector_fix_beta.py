...
def count_rules(lines):
    # Теперь мы не считаем технические заголовки типа [Adblock Plus 2.0] за правила
    return sum(
        1
        for ln in lines
        if ln.strip() 
...
def fetch_and_clean_filters(input_file, output_file, exceptions, proxies):
    ...
    for src in file_list:
        ...
        for line in lines:
            ...
            # Исключаем обычные текстовые комментарии, но оставляем все валидные 
            # косметические правила AdGuard/uBlock (##, #@#, #?#, #%#, #$# и т.д.)
...
def update_target_file(target_file, cleaned_filters, exceptions):
    ...
    while i < len(lines):
        ...
        if s.startswith("!") and not s.startswith("!!"):
            ...
            if name in cleaned_filters:
                # 1. Добавляем сам заголовок категории
                updated_lines.append(line)
                i += 1

                # 2. Сохраняем комментарии/описания (если они идут сразу под заголовком)
                while i < len(lines) and lines[i].strip().startswith("!") and not lines[i].strip().startswith("!!"):
                    updated_lines.append(lines[i])
                    i += 1

                # 3. Пропускаем пустые строки перед старыми правилами, чтобы избежать визуальных дыр
                while i < len(lines) and not lines[i].strip():
                    i += 1

                # 4. Добавляем новые актуальные правила
                for rule in cleaned_filters[name]:
                    updated_lines.append(f"{rule}\n")

                print(f"👍 Категория обновлена: {name}")

                # 5. Пропускаем ВСЕ СТАРЫЕ правила этой категории, останавливаясь на 
                # пустой строке ИЛИ начале следующей категории/комментария
                while i < len(lines):
        ...
        # Проверка исключений для правил из тех категорий, которые не обновляются напрямую из файла
    ...
    # Считаем точное количество фильтров
