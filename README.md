...
- [ ] Полный переезд на Codeberg. Репозиторий на GitHub останется и будет обновляться в режиме зеркала. Ссылки на списки фильтров, хостов и другие файлы будут использовать GitHub для обеспечения совместимости, удобства и снижения возможной нагрузки на сервера Codeberg. 
- [ ] Улучшение README, уменьшение... "подгорания 5 точки в описаниях"

> [!WARNING]
> NoADS_RU постепенно переезжает на Codeberg ([новый репозиторий](https://codeberg.org/Zalexanninev15/NoADS_RU)), однако, чтобы обеспечить совместимость и удобство, репозиторий на Github останется и будет автоматически обновляться в соответствии с новым репозиторием (зеркало).
...
| 📹 **NoADS_RU YouTube** | Несамостоятельный список фильтров | Фильтры конкретно для YouTube. |
...
| 👨‍💻 **NoADS_RU Faust** | Несамостоятельный список фильтров | Фильтры от Faust'а. |
...
Список фильтров на базе **NoADS_RU Hosts BlockerFL**
...
> [!NOTE]
> ⚠️ Важно! Это не самостоятельные списки фильтров, как остальные, которые развиваются в рамках NoADS_RU. Данный список фильтров как-бы "наслаивается" на другие списки. Такой подход позволяет не выбирать какой-то один список фильтров в качестве основы и планируется использовать для всех списков фильтров, чтобы вас было удобнее кастомизировать списки фильтров под свои нужды.
...
Upd. Автор ведёт активные поиски обновлённой информации, а также готовит NoADS_RU Web Unlimited CatNG.
...
Что-то не работает? Чего-то не хватает? Пожалуйста, напишите об этом [здесь](https://codeberg.org/Zalexanninev15/NoADS_RU/issues/new), но лучше через [другие каналы связи](https://z15.neocities.org/contacts) (email, беседа в Delta Chat).
