...
> Для Chromium подобных браузеров рекомендуется ставить расширение через файл, а не через Магазин расширений, т.к. это позволит сохранить доступ к расширению в случае его удаления в будущем (что точно произойдёт). Использование unlock Origin Lite автор не одобряет, да и режет рекламу он хуже, но если альтернатив нет - можете использовать.
