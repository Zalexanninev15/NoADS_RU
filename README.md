...
Списки фильтров, в которых автор сделал некоторые послабления в плане блокировки доменов и аргументов для сайтов (доступен тот же *max.ru*, выполнение скриптов VK API и Yandex API). В **NoADS_RU Extended Evil** и **NoADS_RU Extended Plus Evil** работают также панели в Google сервисах (выбор сервисов гугла и смена/управление аккаунтом), которые работают как в обычном **NoADS_RU**, так и в **NoADS_RU Evil**. Данные списки фильтров будут расширяться не так часто.
