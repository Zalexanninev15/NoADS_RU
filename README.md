...
2. При написании обращения обязательно точно укажите в каком списке фильтров или в списке хостов произошла проблема! Отправленные обращения без указания точного NoADS_RU не будут рассматриваться и будут удалены!
> [!TIP]
...
> - `TGCL` - NoADS_RU TGCL
3. При написании обращения обязательно точно укажите полную ссылку на проблемный сайт! Отправленные обращения без указания точного URL не будут рассматриваться и будут удалены!
4. Перед отправкой обращения крайне рекомендуется (но не обязательно) самостоятельно идентифицировать причину поломки (приложите конкретный фильтр или домен, который ломает работу сайта), чтобы автору было проще внедрить исправление.
...
5. Если что-то не работает, то также может помочь переподключение списка в настройках блокировщика или выбор более простого варианта списка (НЕ Extended). Если это хосты, то подойдёт более простой [Блокировщик](https://raw.githubusercontent.com/Zalexanninev15/NoADS_RU/main/hosts/blocker.txt).
6. Если какой-то сайт не открывается и явно блокируется при помощи списка: https://github.com/Zalexanninev15/NoADS_RU/pull/24#issuecomment-3262847701
---
