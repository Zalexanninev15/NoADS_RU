...
Блокировка, скрытие рекламы, скрытие root прав, модификации для Android и в приложениях. В первую очередь данный проект нацелен на устройства с Root правами и Vector (LSposed), однако для non-root девайсов ~возможно использовать через LSPatch или~ (не уверен точно, что это актуально ещё) имеются готовые APK.
