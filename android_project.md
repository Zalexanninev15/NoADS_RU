...
Блокировка, скрытие рекламы, скрытие root прав, модификации для Android и в приложениях. В первую очередь данный проект нацелен на устройства с Root правами и Vector (LSposed), однако для non-root девайсов ~возможно использовать через LSPatch или~ (не уверен точно, что это актуально ещё) имеются готовые APK. Также в скором времени я займусь созданием модуля, в основном для Magisk, для использования хостов на Android устройствах. Первая альфа-версия расположена в директории `hosts`.

> [!TIP]
> Если выбираете себе новый смартфон и хотите получить в дальнейшем root доступ – присмотритесь к [данному репозиторию](https://github.com/zenfyrdev/bootloader-unlock-wall-of-shame?tab=readme-ov-file#ℹ%EF%B8%8F-safe-for-now-trollface). Представлены производители смартфонов, которых стоит избегать, сложно разблокировать загрузчик и "безопасные производители", которые пока нормально относятся к разблокировке загрузчика на своих смартфонах и получению root-доступа. Таким образом из большого числа устройств я остановился на OnePlus 15 (16/512, глобалка) на замену моего Realme GT Neo 3 (12/256, 80W, CN).
...
* [NoVPNDetect Enhanced](https://github.com/BlueCat300/NoVPNDetectEnhanced) - скрытие различных локальных обнаружений VPN через публичный API от приложений на android устройствах. Работает для RU приложений, их необходимо выбрать как обычно в менеджере LSPosed.
* [Marketplace AdAway](://github.com/Xposed-Modules-Repo/ru.bluecat.marketplaceadaway) - убирает рекламу и скрывает лишнее в Wildberries, Ozon, Яндекс Маркет, Магнит Маркет (в составе магазинного приложения)
...
Проект переименован в NexAlloy и переехал [сюда](https://github.com/NexAlloy/NexAlloy). Поддержка приложений: Youtube, Google Photos, Youtube Music, Instagram, Threads, Strava, Photomath, AllTrails, Reddit.
...
* [Любителям ТикТока (ранее автор публиковал и другие модификации)](https://t.me/alexey070315)
...
Также могу предложить посмотреть другие Telegram каналы (также годами пользуюсь), модификации встречаются очень уникальные, но здесь уже хотя бы какая-то проверка нужна:
* [FreeApkexe](https://t.me/FreeApkexe) - много крутых модов, имеются уникальные, но много аналитики и другой метрики остаётся от разработчиков
* [EasyAPK](https://t.me/EasyAPK) - много крутых модов, имеются уникальные + бот-каталог
* [powerga](https://t.me/powerga) - для игр
